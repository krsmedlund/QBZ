...
RenderProgram::RenderProgram(QString name, bool is_pp)
{
    ...
    inPortModelData = network::registerInPort(name.toStdString(), "ModelData", 1, true);
    outPortRenderProgram = network::registerOutPort(name.toStdString(), "RenderProgram", this);
}


void RenderProgram::Do()
{
    Model *m = (Model*)inPortModelData->getValuePtr();
    drawObject(m);
}
