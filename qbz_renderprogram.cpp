...
RenderProgram::RenderProgram(QString name, bool is_pp) {
    ...
    this->program = ShaderStore::open()->get_or_load_shader(name);
...
// We can have any number of models to draw
// if there is no models we will draw our own fullscreen quad with textures from the texture in port.

void RenderProgram::drawObjects() {

    /* iterate our objects */
}

void RenderProgram::drawObject(Model* model, Camera* cam, Light* lights[], int numLights) {
